...
def _unwrap(result):
    """Parse list[TextContent] from call_tool() back to a dict."""
...
async def tool1_cs3_called():
    ...
    with patch.object(mcp_server.cs3_client, "get_assessment", return_value=mock_assessment) as mock:
        result = _unwrap(await call_tool("calculate_org_air_score", {"company_id": "NVDA"}))
...
async def tool2_basic():
    result = _unwrap(await call_tool("get_company_evidence", {"company_id": "NVDA", "limit": 5}))
    assert "error" not in result, f"Got error: {result}"
    assert "evidence" in result
    print(f"        evidence items returned: {result.get('count', len(result.get('evidence', [])))}")

async def tool2_with_dimension():
    result = _unwrap(await call_tool("get_company_evidence", {"company_id": "JPM", "dimension": "talent", "limit": 3}))
    assert "evidence" in result
...
async def tool3_basic():
    result = _unwrap(await call_tool("generate_justification", {"company_id": "NVDA", "dimension": "talent"}))
    assert "error" not in result, f"Got error: {result}"
...
async def tool4_basic():
    result = _unwrap(await call_tool("project_ebitda_impact", {
...
async def tool5_basic():
    result = _unwrap(await call_tool("run_gap_analysis", {"company_id": "NVDA", "target_org_air": 90.0}))
    assert "error" not in result, f"Got error: {result}"
...
async def tool6_basic():
    result = _unwrap(await call_tool("get_portfolio_summary", {"fund_id": "PE-FUND-I"}))
    assert "error" not in result, f"Got error: {result}"
